...
router.put('/lessons/:lessonId', async (req, res: Response, next: NextFunction) => {
  try {
...
router.patch('/lessons/:lessonId/status', async (req, res: Response, next: NextFunction) => {
  try {
    const { lessonId } = LessonIdParamSchema.parse(req.params)
    const { sub } = (req as unknown as AuthRequest).user
...
router.get('/overview', async (req, res: Response, next: NextFunction) => {
  try {
...
router.get('/levels/:levelId/unlock-status', async (req, res: Response, next: NextFunction) => {
  try {
...
router.post('/initialize', async (req, res: Response, next: NextFunction) => {
  try {
    const { sub } = (req as AuthRequest).user
