...
| `source_story_zh` | 是 | 完整中文源文；字段名保留为 `source_story_zh`，但正文风格应为纪录片式专题解说，再拆成 `sentences` |
...
- 禁止的反模式包括但不限于：
  ...
  - 把不同文章共用一段开场白或收束语。
- 一篇文章的正文必须像纪录片旁白或专题解说：围绕标题直接介绍对象、背景、形成原因、典型特征、地域或时间脉络、文化影响和学习价值；可以有画面感，但不得为了“故事化”强行安排人物、对话、动机、冲突或情节转折；120 句一气呵成可朗读，**任何两篇文章不得使用相同的句式骨架反复出现**。
...
发现中国文章必须先生成完整中文源文，再拆句和多语言适配。字段名仍为 `source_story_zh`，但内容不是强行情节故事，而是纪录片式介绍文。禁止直接用句子模板、事实卡片、问答卡或 SEO/GEO 片段拼成正文。

```jsonc
"source_story_zh": [
  "中国朝代顺序口诀把主要王朝按时间串联起来，帮助学习者先看见历史的基本轮廓。",
  "它不是一张详细年表，而是一条方便记忆的时间线。"
]
```

| 步骤 | 要求 |
|------|------|
| 1. 完整中文源文 | 先写一篇可从头读到尾的纪录片式介绍文，首期固定 120 个中文句子 |
| 2. 中文断句 | `source_story_zh[n]` 必须与 `sentences[n].content_zh` 完全一致 |
| 3. 拼音生成 | 拼音只从 `content_zh` 生成，不允许手写错配或复用旧句拼音 |
| 4. 多语言适配 | `content_en/vi/th/id` 必须逐句对应中文源文语义，不允许留空、占位或泛化 |
| 5. 元数据后置 | SEO/GEO 只能从正文中抽取，不得反向生成正文句子 |
