...
const themeLabels: Record<TrackCode, string> = { daily: '日常', ec: '电商', factory: '工厂', hsk: 'HSK' };
...
function themeLabel(value: TrackCode | string) {
  return themeLabels[value as TrackCode] ?? titleCase(value === 'ec' ? 'ecommerce' : value);
}
...
function CourseOnboarding({ navigate, loggedIn }: { navigate: (path: string) => void; loggedIn: boolean }) {
  ...
  return <section className="page course-page stack"><PageTitle eyebrow="CR-FR-001" title="课程欢迎引导" subtitle="选择目标、水平，系统推荐 1-2 个主题；可跳过进入 Dashboard 自选。" /><div className="course-split"><Card className="glass-panel"><Segmented label="学习目标" items={['HSK 备考', '工作 / 工厂', '电商采购', '兴趣日常']} value={goal} onChange={setGoal} /><Segmented label="当前水平" items={['零基础', 'HSK 1-3', 'HSK 4-6', 'HSK 7+']} value={level} onChange={setLevel} /><div className="course-actions"><Button onClick={recommend}><Route size={18} />推荐主题</Button><Button variant="secondary" onClick={() => navigate('/learn')}>跳过</Button></div></Card><Card className="glass-panel"><h2>推荐结果</h2>{result ? <div className="recommend-list">{(result.tracks as string[]).map((track) => <button key={track} onClick={() => navigate(`/learn/${track}`)}><Badge>{track}</Badge><strong>{themeLabel(track)}</strong><small>起始阶段 {String(result.startStage)} · {result.pinyinRequired ? '建议先拼音入门' : '可直接进入主题'}</small></button>)}</div> : <p>点击推荐后展示 1-2 个主题。</p>}<Button disabled={!result || !loggedIn} onClick={confirm}><CheckCircle2 size={18} />确认并写入报名</Button>{!loggedIn ? <Toast type="warning">登录后可保存多主题报名和跨设备进度。</Toast> : null}</Card></div></section>;
}
...
function PaywallPanel({ track, stage, navigate, loggedIn }: { track: TrackCode; stage: number; navigate: (path: string) => void; loggedIn: boolean }) {
  ...
  return <Card className="paywall glass-panel"><LockKeyhole size={24} /><h2>解锁阶段 {stage}</h2><p>免费试学开放每个主题 Stage 1-3 的全部章节；跨级购买允许直接买任意主题任意阶段。</p><div className="price-grid">{priceOptions.map((option) => <button key={option.type} disabled={!loggedIn} onClick={() => buy(option.type)}><strong>{option.label}</strong><small>{option.detail}</small></button>)}</div><Button variant="secondary" onClick={() => navigate(`/learn/${track}/1/1`)}>回到免费范围</Button>{message ? <Toast type="success">{message}</Toast> : null}{!loggedIn ? <Toast type="warning">请先登录再购买或记录学习完成度。</Toast> : null}</Card>;
}
